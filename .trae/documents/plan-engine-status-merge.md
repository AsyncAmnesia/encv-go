...
2. 新建 `EngineDetail.vue` 二级页面，提供完整的引擎运行时状态和构建信息
...
- AboutDetail.vue：BuildInfo 展示（FFmpeg 版本、x264、编解码器列表等手风琴展开）

## 实施步骤

### 步骤 1：Settings.vue — 合并引擎状态到联机组

1. 在"联机"组（L83-100）的服务器入口下方，新增一个"引擎状态"入口项：
...
3. 删除不再需要的 CSS 类：`.engine-error-inline`、`.engine-detail-text`、`.status-badge`（如果有的话）
...
<ion-page>
  ...
  <ion-content>
    ...
    <ion-list header="构建信息" v-if="buildInfo">
      <ion-item> FFmpeg 版本 + codename </ion-item>
      <ion-item> x264 版本 </ion-item>
      <ion-item> NDK 版本 + API Level </ion-item>
...
样式参考：复用 AboutDetail.vue 的手风琴 + tag-list 样式模式
...
1. **不改动 AboutDetail.vue**：AboutDetail 的 BuildInfo 展示保持不变，它侧重"关于/第三方库"视角；EngineDetail 侧重"引擎运行时 + 构建配置"视角，两者信息有重叠但定位不同
2. **运行时状态与构建信息分离**：运行时状态（FFmpeg/FFprobe 是否可加载）是动态的，构建信息是静态的，分两个 section 展示更清晰
3. **联机组概览只显示摘要**：Settings 联机组中的引擎入口只显示 FFmpeg/FFprobe 的 ✅/❌ badge，不显示错误详情（详情在二级页面）
4. **组件列表复用 AboutDetail 的 tag-list 样式**：保持视觉一致性
