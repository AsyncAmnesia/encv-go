...
## 十一、严禁下载 / 拉取第三方项目源码铁律（⚠️ 实战踩坑 2026-06）

> **核心原则：参考资料用引用链接形式，绝不用 curl / WebFetch / WebSearch 下载第三方仓库的源码文件。**
> **这次违规越界：诊断 aar2apk 实现时多次 curl raw.githubusercontent.com 下载 lnzz123/ComboLite 源码，浪费 4 次 WebFetch + 3 次 curl + 1 次 WebSearch，且下载内容并未真正读完。**

### 11.1 禁止行为清单

| ❌ 禁止 | ✅ 正确做法 |
|--------|------------|
| `curl -fsSL https://raw.githubusercontent.com/xxx/yyy/...` 下载完整源码 | 只生成 markdown 链接，让用户去查 |
| `WebFetch` 拉第三方仓库的 `.kt` / `.java` / `.gradle` 等源码文件 | 引用 URL 进规则文件，不抓取内容 |
| `WebSearch` 搜出第三方仓库具体代码实现 | 搜概念 / 文档 / issue 摘要，**不要**搜具体源码 |
| `wget` / `git clone` 完整第三方仓库 | 已 clone 的源码（`/tmp/combolite-src`）才能读 |

### 11.2 允许的工具场景

| 工具 | 允许场景 | 禁止场景 |
|------|---------|---------|
| `WebSearch` | 搜概念、文档、issue 摘要、API 行为、StackOverflow | 搜「xxx 仓库 yyy 文件源码」「xxx 函数实现」 |
| `WebFetch` | 抓 README / issue / blog / official doc | 抓 `raw.githubusercontent.com/.../*.kt` 等源码 |
| `curl` | 查 maven published artifact metadata / 校验下载 / GitHub API 元数据 | 抓 raw 源码文件 |
| `WebSearch + 链接` | 引用第三方仓库做参考（`lnzz123/ComboLite` demo） | 抓内容后大段复制进代码 / 规则 |

### 11.3 已 clone 的本地源码

`/tmp/combolite-src/com/combo/core/` 目录里**已经 clone 过**的 ComboLite 源码，**可以自由 read / grep**：

- `api/IPluginService.kt` / `IPluginReceiver.kt` / `IPluginProvider.kt`
- `component/service/BaseHostService.kt` / `BasePluginService.kt`
- `component/receiver/BaseHostReceiver.kt` / `BasePluginReceiver.kt`
- `component/provider/BaseHostProvider.kt` / `BasePluginProvider.kt`
- `component/activity/BaseHostActivity.kt` / `BasePluginActivity.kt`
- `proxy/ProxyManager.kt`
- `runtime/loader/PluginClassLoader.kt` / `runtime/installer/XmlManager.kt`
- `utils/Extensions.kt`
- `app/src/main/java/com/combo/plugin/sample/HostApp.kt`（demo）
- `app/src/main/AndroidManifest.xml`（demo manifest）

**任何研究 ComboLite 行为** → **先查这个目录**。本地有就别下载。

### 11.4 真要查第三方源码时的正确路径

1. **先**在 `/tmp/combolite-src` 找 → 有就直接 read
2. **再**生成 markdown 链接进规则 / 文档，让用户去查
3. **最后**才考虑 `git clone`（但先问用户，因为 clone 整个 repo 很重）
4. **绝不用** `curl raw.githubusercontent.com` 单文件下载

### 11.5 越界案例（2026-06-03 本会话）
...
curl -fsSL https://api.github.com/repos/lnzz123/ComboLite/...                    ❌
curl -fsSL https://raw.githubusercontent.com/lnzz123/ComboLite/master/.../xxx.kt  ❌
```

**问题**：
- 6+ 次外部请求，浪费 token + 延迟
- 下载内容**没**真正读完就放弃（aar2apk PluginAar2ApkPlugin.kt 多次失败未成功抓）
- **没产出**有价值的修复方案——其实 `aar2apk` 只是把 plugin AAR 转 plugin APK（dex 合并），不需要看源码就能推断

**反思**：
- 用户问的运行时 `Failed resolution of: Lopenlistlib/Event;` 是类加载问题
- 根因分析**不需要**看 aar2apk 源码——只用想清楚「AAR → APK 时 dex 合并是否含 openlistlib 类」即可
- 想清楚后修复路径：**让 openlist-classes.jar 真的存在**（CI cache miss 兜底 / 强制 Extract 步骤不 swallow error）

### 11.6 铁律优先级

这条铁律**高于**一切「快速诊断」「深挖细节」的诱惑：

- **不确定行为** → 优先查本地 clone (`/tmp/combolite-src`) + 现有规则文件 + 现有代码
- **还不确定** → 引用 URL 进规则文件，**问用户**「你能帮我看下 xxx 仓库 yyy 文件吗」
- **绝对不要** → curl / WebFetch raw.githubusercontent.com / WebSearch 搜源码
