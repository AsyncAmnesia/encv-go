# 本地 Kotlin 编译验证规则（Local kotlinc Validation）

> 来自踩坑：曾两次 CI 编译失败（`OpenListNativeService.kt:161:21 'return' is prohibited here`），每次 CI 往返 5+ 分钟。
> 之后用户原话："**本身跑kotlin验证啊**"——所有 Kotlin 改动都应**先本地 kotlinc 验证**再 push。
...
**`OpenListNativeService.kt` L130-220**（Phase 27 修复历史）：
...
| 主题 | 文档 |
|------|------|
| 完整 Android 编码标准 | [.trae/rules/kotlin-android.md](kotlin-android.md) |
| 验证纪律（核实先于生成） | [.trae/rules/verification-discipline.md](verification-discipline.md) |
| Phase 27 集成背景 | [.trae/specs/build-openlist-fork-as-android-native/spec.md](../specs/build-openlist-fork-as-android-native/spec.md) |
