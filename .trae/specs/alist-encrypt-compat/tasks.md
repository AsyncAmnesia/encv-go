...
- [ ] 13.1: `useAlistEncrypt.ts` L27 — 移除 `|| file.isEncrypted` 条件，只保留 `file.isDirectory` 检查 + **配置后缀**匹配
- [ ] 13.2: 更新测试：测试用例中使用**变量化的后缀值**（如 `TEST_SUFFIX = '.bin'`），不硬编码在断言描述中
...
- [ ] 17.1: 扩展 `__tests__/features.alist-encrypt.test.ts`：
  - isAlistEncrypted 对 isEncrypted=true 非 alist-encrypt 后缀文件返回 false
  - getAlistActions 对 isEncrypted=true 文件返回 decrypt action（新增分支 B）
  - getAlistActions 三分支全覆盖（使用动态后缀常量）
