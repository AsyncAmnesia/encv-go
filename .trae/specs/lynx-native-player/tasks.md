...
- [ ] Task 1: 初始化 Lynx 播放器子项目
  - [ ] SubTask 1.1: 创建 `encv-mobile/lynx-player/` 目录，初始化 package.json，安装 @lynx-js/react、rspeedy 等依赖
  - [ ] SubTask 1.2: 配置 rspeedy.config.ts，设置构建输出为 `dist/player.lynx.bundle`
  - [ ] SubTask 1.3: 创建 `src/typing.d.ts`，声明 VideoPlayerModule 和 GoBackendModule 接口
  - [ ] SubTask 1.4: 创建 `src/App.tsx` 播放器主界面骨架

- [ ] Task 2: 实现 VideoPlayerModule Native Module（Android）
  - [ ] SubTask 2.1: 创建 `VideoPlayerModule.kt`，封装 ExoPlayer 实例管理（play/pause/seekTo/release）
  - [ ] SubTask 2.2: 实现 SurfaceView 嵌入机制，将 ExoPlayer 视频输出嵌入 LynxView 布局
  - [ ] SubTask 2.3: 实现播放状态回调（onIsPlayingChanged、onPlaybackStateChanged、onPlayerError）
  - [ ] SubTask 2.4: 实现全屏切换（setFullscreen）和屏幕方向控制（setOrientation）
  - [ ] SubTask 2.5: 实现 Range 请求流式播放支持（ExoPlayer 默认支持 HTTP Range）
...
- [ ] Task 4: 重写 PlayerActivity 为 LynxView 宿主
  ...
  - [ ] SubTask 4.5: 注册 VideoPlayerModule 和 GoBackendModule 到 LynxEnv

- [ ] Task 5: 实现 Lynx 播放器 UI
  - [ ] SubTask 5.1: 实现 VideoPlayer 组件（视频区域 + SurfaceView 容器 + 控件覆盖层）
  - [ ] SubTask 5.2: 实现 PlayerControls 组件（播放/暂停、进度条、时间、音量、全屏）
...
- [ ] Task 6: 修改 Android 构建配置
  - [ ] SubTask 6.1: 修改 build.gradle.kts，添加 Lynx SDK + ExoPlayer 依赖
  - [ ] SubTask 6.2: 添加 USE_LYNX_PLAYER 构建变体配置
  - [ ] SubTask 6.3: 修改 AndroidManifest.xml PlayerActivity 配置
  - [ ] SubTask 6.4: 修改 post-cap-sync.mjs，添加 Lynx bundle 复制逻辑
...
- [Task 2] depends on [Task 4]（VideoPlayerModule 需要在 PlayerActivity 中注册）
