...
- [ ] [verify] `go test -race ./agent/... -run TestHooks` 通过
...
- [ ] [verify] `go test -race ./agent/... -run TestSessionStore` 通过
...
- [ ] [verify] `pnpm test -- appServerRealtimeReducer` 通过
...
- [ ] [verify] `go test ./agent/... -run TestSkills` 通过
