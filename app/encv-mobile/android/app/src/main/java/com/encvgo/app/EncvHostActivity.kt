...
class EncvHostActivity : BaseHostActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        LogBridge.i(TAG, "onCreate: intent=$intent createTime=$createTime")
        ...
        LogBridge.i(TAG, "onCreate: pluginId=$pluginId targetActivity=$targetActivity")

        try {
            super.onCreate(savedInstanceState)
            proxyStarted = true
            val elapsed = System.currentTimeMillis() - createTime
            LogBridge.i(TAG, "onCreate: super.onCreate completed, proxy should have started (elapsed=${elapsed}ms)")
        } catch (e: Exception) {
            LogBridge.e(TAG, "onCreate: super.onCreate failed: ${e.message}", e)
    ...
    override fun onPostCreate(savedInstanceState: Bundle?) {
        ...
        if (!proxyStarted) {
            LogBridge.w(TAG, "onPostCreate: proxy still not started after ${System.currentTimeMillis() - createTime}ms, scheduling timeout check")
            Handler(Looper.getMainLooper()).postDelayed({
                if (!proxyStarted && !isFinishing && !resultSet) {
                    LogBridge.e(TAG, "Timeout: proxy never started after onPostCreate+5s, finishing with error")
    ...
    override fun onResume() {
        super.onResume()
        val elapsed = System.currentTimeMillis() - createTime
        LogBridge.i(TAG, "onResume: proxyStarted=$proxyStarted elapsed=${elapsed}ms resultSet=$resultSet")

        if (!proxyStarted) {
            LogBridge.w(TAG, "onResume: proxy not started, finishing with error")
        ...
        if (elapsed > PROXY_TIMEOUT_MS) {
            LogBridge.w(TAG, "onResume: proxy started but took ${elapsed}ms (>${PROXY_TIMEOUT_MS}ms), may be stuck")
        }
    }

    override fun onDestroy() {
        if (!resultSet) {
            LogBridge.w(TAG, "onDestroy: result never set (user pressed back or activity killed), setting success=true as default")
        ...
        LogBridge.i(TAG, "onDestroy: proxyStarted=$proxyStarted resultSet=$resultSet elapsed=${System.currentTimeMillis() - createTime}ms")
        super.onDestroy()
    }
    ...
    private fun finishWithResult(
        ...
        LogBridge.e(TAG, "finishWithResult: success=$success error=$error detail=$detail")
        setResult(RESULT_OK, Intent().apply {
