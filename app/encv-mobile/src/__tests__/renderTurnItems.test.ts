...
const tc = (id: string, kind: Message['tool_calls'][number]['kind'], status: 'pending' | 'running' | 'success' | 'failed' = 'success', needsConfirm = false): Message['tool_calls'][number] => ({
  id,
  name: `tool_${id}`,
  args: kind === 'webSearch' ? JSON.stringify({ query: `q-${id}` }) : kind === 'command' ? JSON.stringify({ command: 'ls' }) : JSON.stringify({ path: `/tmp/${id}` }),
