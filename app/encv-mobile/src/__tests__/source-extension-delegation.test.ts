...
function setupExtensionsData(extensions: Record<string, string>, conflicts?: ContainerExtensionsResponse['conflicts']): ContainerExtensionsResponse {
  return {
    extensions,
    conflicts: conflicts ?? [],
  }
    ...
    it('同一扩展名 .ts 在不同 MIME 下应路由到不同插件', async () => {
      ...
      function resolvePlugin(sourceExt: string, mimeType: string): string | null {
        const matched = plugins.filter((p) =>
          p.supportedSourceExts.includes(sourceExt.toLowerCase()) &&
          p.supportedMimePrefixes.some((prefix) => mimeType.startsWith(prefix))
        )
