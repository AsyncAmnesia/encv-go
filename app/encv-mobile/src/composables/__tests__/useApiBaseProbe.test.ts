...
import { __resetApiBaseProbeForTest, useApiBaseProbe } from '@/composables/useApiBaseProbe'
 ...
 */
function setupFetchMock(handlers: Array<{
 ...
 */
function setupFetchMockWithRejects(handlers: Array<{
  match: (url: string) => boolean
  respond?: () => Response
  reject?: () => never
}>) {
...
beforeEach(() => {
  ...
  vi.spyOn(encv, 'setApiBaseUrl').mockImplementation((url: string) => {
    try { localStorage.setItem('encv-server-url', url) } catch { /* ignore */ }
  })
...
describe('useApiBaseProbe — 优先级链', () => {
  it('[1] cached 命中 → 立刻返回，不试 loopback', async () => {
    ...
    setupFetchMock([
      { match: (u) => u.includes('192.168.1.5'), respond: () => okResponse() },
    ])
  ...
  it('[3] cached 失败 + loopback 失败 + LAN 候选命中 → 走 lan-candidate', async () => {
    ...
    setupFetchMock([
      { match: () => true, respond: () => new Response('', { status: 502 }) },
    ])
    ...
    setupFetchMockWithRejects([
      { match: (u) => u.includes('192.168.1.99'), respond: () => new Response('', { status: 502 }) },
      { match: (u) => u.includes('127.0.0.1'), reject: () => { throw new TypeError('Failed to fetch') } },
  ...
  it('[4] 全部失败 → 抛 all-candidates-failed，不写 localStorage', async () => {
    localStorage.setItem('encv-server-url', 'http://192.168.1.99:2025')
    setupFetchMockWithRejects([
      { match: () => true, reject: () => { throw new TypeError('Failed to fetch') } },
    ])
    const probe = freshProbe()
