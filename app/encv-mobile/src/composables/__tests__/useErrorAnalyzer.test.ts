...
import { analyzeError, CATEGORY_META } from '@/composables/useErrorAnalyzer'
...
describe('CATEGORY_META', () => {
  it('所有 category 都有 meta', () => {
    const cats = [
      'network', 'auth', 'not_found', 'server_error', 'unsupported_version',
      'unsupported_cipher', 'unsupported_compression', 'wrong_password',
      'file_not_found', 'permission_denied', 'oom', 'timeout',
      'plugin_crash', 'invalid_request', 'unknown',
    ] as const
    for (const c of cats) {
