...
import { usePinchZoom, mockTouchList } from '@/composables/usePinchZoom'
...
describe('usePinchZoom - 双指距离变化', () => {
  ...
  it('TestPinch_PinchOut_Increases: 双指距离变大 → zoomScale 变大', () => {
    ...
    pz.__onTouchStart(
      makeTouchEvent({ touches: [{ clientX: 0, clientY: 0 }, { clientX: 100, clientY: 0 }] }) as unknown as TouchEvent
    )
    expect(pz.zoomScale.value).toBe(1.0) // 起始不变

    // 双指移动：距离 = 200 → ratio = 2 → scale = 1.0 * 2 = 2.0 → clamp 1.5
    pz.__onTouchMove(
      makeTouchEvent({ touches: [{ clientX: 0, clientY: 0 }, { clientX: 200, clientY: 0 }] }) as unknown as TouchEvent
    )
    expect(pz.zoomScale.value).toBe(1.5)
  })
  ...
  it('TestPinch_PinchIn_Decreases: 双指距离变小 → zoomScale 变小', () => {
    ...
    pz.__onTouchMove(
      makeTouchEvent({ touches: [{ clientX: 0, clientY: 0 }, { clientX: 50, clientY: 0 }] }) as unknown as TouchEvent
    )
    expect(pz.zoomScale.value).toBe(0.5)
  })
  ...
  it('TestPinch_Pinch_ClampUpper: 双指放大到 3 倍 → clamp 1.5', () => {
    ...
    pz.__onTouchMove(
      makeTouchEvent({ touches: [{ clientX: 0, clientY: 0 }, { clientX: 300, clientY: 0 }] }) as unknown as TouchEvent
    )
    expect(pz.zoomScale.value).toBe(1.5)
  })
  ...
  it('TestPinch_Pinch_ClampLower: 双指缩小到 0.1 倍 → clamp 0.5', () => {
    ...
    pz.__onTouchMove(
      makeTouchEvent({ touches: [{ clientX: 0, clientY: 0 }, { clientX: 10, clientY: 0 }] }) as unknown as TouchEvent
    )
    expect(pz.zoomScale.value).toBe(0.5)
  })
  ...
  it('TestPinch_Move_SingleFinger: 单指 move → 忽略', () => {
    ...
    // 单指 move
    pz.__onTouchMove(
      makeTouchEvent({ touches: [{ clientX: 50, clientY: 50 }] }) as unknown as TouchEvent
    )
    expect(pz.zoomScale.value).toBe(1.0)
  })
})
