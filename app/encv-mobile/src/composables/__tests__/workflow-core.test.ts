...
import type { StepStatus, StepRun, JobRun } from '@/lib/workflow/types'
import { resolveExecutionOrder, getNextReadyJobs } from '@/lib/workflow/scheduler'
import { expandMatrix } from '@/lib/workflow/matrixExpander'
...
describe('inferWorkflowStatus', () => {
  ...
  it('全 success → success', () => {
    expect(inferWorkflowStatus(makeJobs([
...
describe('evaluateCondition', () => {
  ...
  it('eq 匹配', () => {
    expect(
      evaluateCondition({ op: 'eq', left: '${{type}}', right: 'encrypt' }, { vars: { type: 'encrypt' } }),
    ).toBe(true)
  })

  it('eq 不匹配', () => {
    expect(
      evaluateCondition({ op: 'eq', left: '${{type}}', right: 'decrypt' }, { vars: { type: 'encrypt' } }),
    ).toBe(false)
  })

  it('neq', () => {
    expect(
      evaluateCondition({ op: 'neq', left: 'a', right: 'b' }, {}),
    ).toBe(true)
  })
  ...
  it('and 全 true → true', () => {
    expect(
      evaluateCondition(
        { op: 'and', children: [{ op: 'always' }, { op: 'always' }] },
        {},
      ),
    ).toBe(true)
  })
  ...
  it('and 有 false → false', () => {
    expect(
      evaluateCondition(
        { op: 'and', children: [{ op: 'always' }, { op: 'eq', left: 'a', right: 'b' }] },
        {},
      ),
    ).toBe(false)
  })
  ...
  it('or 有 true → true', () => {
    expect(
      evaluateCondition(
        { op: 'or', children: [{ op: 'always' }, { op: 'eq', left: 'a', right: 'b' }] },
        {},
      ),
    ).toBe(true)
  })
