...
describe('readTurnStatus', () => {
  describe('active 集合', () => {
    it.each([
      'active', 'inprogress', 'running', 'editing',
      'thinking', 'in_progress', 'streaming',
    ])('%s → "active"', (input) => {
  ...
  describe('interrupted 集合', () => {
    it.each([
      'interrupted', 'interrupt', 'canceled', 'cancelled',
    ])('%s → "interrupted"', (input) => {
  ...
  describe('类型契约', () => {
    it('返回值始终是 ActiveStatus 联合类型之一', () => {
      const values: unknown[] = [
        'running', 'done', 'failed', 'cancelled', 'idle', '', null, undefined, 42, true, [],
        { type: 'running' }, { status: 'failed' }, { state: 'done' }, { kind: 'cancelled' },
      ]
