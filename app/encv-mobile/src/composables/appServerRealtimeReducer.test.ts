...
describe('readRealtimeCacheVersion', () => {
  ...
  it('falls back to params / approval / top-level', () => {
    expect(
      readRealtimeCacheVersion({ params: { cacheVersion: 5 } }),
    ).toBe(5)
    expect(
      readRealtimeCacheVersion({ approval: { cacheVersion: 7 } }),
    ).toBe(7)
    expect(
      readRealtimeCacheVersion({ cacheVersion: 9 }),
    ).toBe(9)
  })
  ...
  it('returns null for NaN / Infinity / unparseable strings', () => {
    expect(
      readRealtimeCacheVersion({ payload: { cacheVersion: NaN } }),
    ).toBeNull()
    expect(
      readRealtimeCacheVersion({ payload: { cacheVersion: Infinity } }),
    ).toBeNull()
    expect(
      readRealtimeCacheVersion({ payload: { cacheVersion: 'not-a-number' } }),
    ).toBeNull()
  })
...
describe('readRealtimeServerInstance', () => {
  it('returns serverInstanceId for type=connected', () => {
    expect(
      readRealtimeServerInstance({ type: 'connected', serverInstanceId: 'srv-1' }),
    ).toBe('srv-1')
  })

  it('returns empty string for non-connected events', () => {
    expect(
      readRealtimeServerInstance({ type: 'message', serverInstanceId: 'srv-1' }),
    ).toBe('')
    expect(
      readRealtimeServerInstance({ type: 'stream_end', serverInstanceId: 'srv-1' }),
    ).toBe('')
    expect(
      readRealtimeServerInstance({ type: 'text_delta', serverInstanceId: 'srv-1' }),
    ).toBe('')
  })
