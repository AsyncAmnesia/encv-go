...
import { normalizeReasoningEffort, i18nKeyFor } from './reasoningEffort'
...
describe('normalizeReasoningEffort', () => {
  ...
  describe('low 集合别名', () => {
    it.each(['low', 'LOW', 'Low', 'minimal', 'MINIMAL', 'small', 'Small'])(
      '%s → "low"',
      (input) => {
  ...
  describe('medium 集合别名', () => {
    it.each(['medium', 'MEDIUM', 'med', 'Med', 'normal', 'Normal'])(
      '%s → "medium"',
      (input) => {
  ...
  describe('high 集合别名', () => {
    it.each(['high', 'HIGH', 'High', 'large', 'LARGE', 'Large'])(
      '%s → "high"',
      (input) => {
  ...
  describe('类型契约', () => {
    it('返回值始终是 string（不会抛错）', () => {
      const values: unknown[] = [
        ...
        'minimal', 'med', 'large', 'extra', 'max',
        'LOW', 'Extra Max', 'extra-max', 'extra_max',
        '', null, undefined, 0, 1, true, false, [], {},
        'unknown', 'idle', 'thinking',
      ]
