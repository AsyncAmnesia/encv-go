...
const RULES: ErrorRule[] = [
  ...
  // 网络层失败
  {
    ...
    fixes: [
      ...
      { title: '真机 native fetch 限制', detail: 'useProxiedFetch.ts 在 native 模式下替换了原生 fetch，确认 headers 已带 Accept: text/event-stream。' },
    ],
  },
  ...
  // 不支持的版本
  {
    ...
    keywords: ['unsupported version', 'version not supported', 'invalid version', 'unknown version', 'version 2', 'version 3', 'deprecated'],
    exclude: ['cipher', 'compression'],
 ...
 */
export function analyzeError(rawMessage: string, options?: {
  httpStatus?: number
  phase?: ErrorPhase
}): ErrorAnalysis {
