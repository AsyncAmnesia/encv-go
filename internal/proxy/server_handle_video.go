...
// 专门处理视频容器
func handleVideoContainer(ctx context.Context, w http.ResponseWriter, containerURL string, headers map[string]string, originalPath string) {
	...
	provider := func(subChunkInfo types.SubChunkInfo) (io.ReadCloser, error) {
		...
		subChunkFileInfo, err := OpenListGetFileURL(subChunkOriginalPath, cfg.Proxy.OpenListHost, cfg.Proxy.Token)
		if err != nil {
	...
	if err := serveDecryptedStreamFromReader(ctx, w, streamingReader, index, false); err != nil {
