...
// 2026-06-14: 删除文件版心跳（StartHeartbeatLoop / writeHeartbeat /
// touchHeartbeatFile）。改为 Server.startHeartbeatLoopInMemory() 维护
// 内存心跳，Kotlin 改用 HTTP /health JSON 探活。
...
func (c *workerClient) RunWithOutput(ctx context.Context, args []string) ([]byte, string, int, error) {
	...
	if runErr != nil {
		...
		if stdout == "" {
			// 🆕 2026-06-14：删除 writeHeartbeat() 调用。Kotlin 已切到 HTTP /health 探活。
	...
	// 🆕 2026-06-14：删除 writeHeartbeat() 调用。Kotlin 已切到 HTTP /health 探活，
	// 不再依赖文件 mtime。Server 端的 startHeartbeatLoopInMemory() 维护内存心跳，
	// Kotlin EncvGoService.checkHeartbeatOk() 读 HTTP JSON。
