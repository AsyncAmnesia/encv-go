...
func BenchmarkVideoPlugin_Encrypt(b *testing.B) {
	...
	for _, videoPath := range videos {
		...
		b.Run(filepath.Base(videoPath), func(b *testing.B) {
			...
			for b.Loop() {
				if err := plugins.EncryptFileWithPlugin(ctx, p, videoPath, filepath.Dir(videoPath), outputDir); err != nil {
					b.Fatalf("加密失败: %v", err)
				}
...
func BenchmarkVideoPlugin_Decrypt(b *testing.B) {
	...
	for _, videoPath := range videos {
		b.Run(filepath.Base(videoPath), func(b *testing.B) {
			...
			if err := plugins.EncryptFileWithPlugin(ctx, p, videoPath, filepath.Dir(videoPath), encryptDir); err != nil {
				b.Fatalf("加密失败: %v", err)
			}
			...
			for b.Loop() {
				...
				if err := plugins.DecryptContainerWithPlugin(ctx, dp, containerPath, decryptDir); err != nil {
					b.Fatal(err)
				}
