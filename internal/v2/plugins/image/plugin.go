...
func (p *ImagePlugin) CanDecrypt(containerPath string) bool {
	...
	if err != nil {
		...
		// 这里的日志可以帮助调试
		// log.Printf("DEBUG: [ImagePlugin.CanDecrypt] Failed to detect kind for '%s': %v\n", containerPath, err)
		return false
	}
...
// 实现 plugins.Plugin 接口
func (p *ImagePlugin) GetContentVirifier() pluginInterfaces.ContentVerifier {
	return nil
}

func (p *ImagePlugin) GroupFiles(inputPaths []string, inputRootDir, outputDir string) ([]string, error) {
...
// Plugin 接口实现
// 执行核心的加密工作
func (p *ImagePlugin) Encrypt(dataReader io.Reader) (*crypto.EncryptionResult, error) {
...
// Plugin 接口实现
// 加密后处理器
func (p *ImagePlugin) PostEncryptProcessor(result *crypto.EncryptionResult) (string, error) {
...
// Plugin 接口实现
func (p *ImagePlugin) Decrypt(containerPath, outputDir string) (string, error) {
