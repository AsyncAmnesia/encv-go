...
func (p *PDFPlugin) GetSettingFields() []pluginInterfaces.SettingField {
	return []pluginInterfaces.SettingField{
		{
			Key:          "ext",
			Type:         "string",
			DefaultValue: ".sccgpdf",
			Help:         "The container file extension for encrypted text files (e.g., '.sccgpdf').",
		},
	}
}
...
// Plugin 接口实现
func (p *PDFPlugin) Initialize(ctx context.Context) error {
	if ctx == p.ctx {
		return nil // 避免重复初始化
	}
	p.ctx = ctx
...
// 实现 plugins.Plugin 接口
func (p *PDFPlugin) GetContentVirifier() pluginInterfaces.ContentVerifier {
	return nil
}

func (p *PDFPlugin) GroupFiles(inputPaths []string, inputRootDir, outputDir string) ([]string, error) {
...
// Plugin 接口实现
// 执行核心的加密工作
func (p *PDFPlugin) Encrypt(dataReader io.Reader) (*crypto.EncryptionResult, error) {
...
// Plugin 接口实现
// 加密后处理器
func (p *PDFPlugin) PostEncryptProcessor(result *crypto.EncryptionResult) (string, error) {
...
// Plugin 接口实现
func (p *PDFPlugin) Decrypt(containerPath, outputDir string) (string, error) {
