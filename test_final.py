"""Final real browser test of the actual user flow."""
from playwright.sync_api import sync_playwright
import json

URL = "http://127.0.0.1:16000/tabs/remote"

def main():
    with sync_playwright() as p:
        browser = p.chromium.launch(
            headless=True,
            args=["--no-sandbox", "--disable-setuid-sandbox"]
        )
        context = browser.new_context(ignore_https_errors=True)
        page = context.new_page()

        requests_log = []
        console_logs = []
        page_errors = []

        page.on("request", lambda req: requests_log.append({
            "type": "request",
            "method": req.method,
            "url": req.url,
            "headers": dict(req.headers) if req.headers else {},
        }))
        page.on("response", lambda resp: requests_log.append({
            "type": "response",
            "method": resp.request.method,
            "url": resp.url,
            "status": resp.status,
            "headers": dict(resp.headers) if resp.headers else {},
        }))
        page.on("console", lambda msg: console_logs.append({
            "type": msg.type, "text": msg.text, "location": msg.location
        }))
        page.on("pageerror", lambda err: page_errors.append(str(err)))

        print(f"Navigating to {URL} ...")
        try:
            page.goto(URL, timeout=30000, wait_until="domcontentloaded")
        except Exception as e:
            print(f"Navigation error: {e}")

        page.wait_for_timeout(8000)
        page.screenshot(path="/tmp/final_page.png", full_page=True)
        print(f"Page screenshot: /tmp/final_page.png")

        try:
            print(f"Page title: {page.title()}")
        except: pass

        frames = page.frames
        print(f"\n=== {len(frames)} frames ===")
        for i, f in enumerate(frames):
            print(f"  Frame[{i}]: {f.url}")

        openlist_frame = next((f for f in frames if "openlist" in f.url.lower()), None)

        if openlist_frame:
            print(f"\n=== OpenList frame: {openlist_frame.url} ===")
            page.wait_for_timeout(5000)
            page.screenshot(path="/tmp/final_with_iframe.png", full_page=True)
            try:
                body_text = openlist_frame.locator("body").inner_text(timeout=8000)
                print(f"\nIframe body text (first 1500 chars):\n{body_text[:1500]}")
            except Exception as e:
                print(f"Get body error: {e}")

        # API requests
        print(f"\n=== API requests ===")
        for entry in requests_log:
            url = entry.get("url", "")
            if "/api/public" in url or "/api/admin" in url:
                if entry.get("type") == "request":
                    h = entry.get('headers', {})
                    interesting = {k: v for k, v in h.items() if k.lower() in
                                   ['content-type', 'origin', 'x-forwarded-prefix']}
                    print(f"  REQ  {entry.get('method')} {url}")
                    if interesting: print(f"        headers: {interesting}")
                elif entry.get("type") == "response":
                    h = entry.get('headers', {})
                    cors = {k: v for k, v in h.items() if k.lower().startswith('access-control')}
                    print(f"  RESP {entry.get('status')} {url}")
                    if cors: print(f"        cors: {cors}")

        # Console logs
        print(f"\n=== Console logs ===")
        for log in console_logs[-30:]:
            print(f"  [{log['type']}] {log['text']}")

        # Page errors
        print(f"\n=== Page errors ===")
        for err in page_errors:
            print(f"  {err}")

        with open("/tmp/final_log.json", "w") as f:
            json.dump({"requests": requests_log, "console": console_logs, "page_errors": page_errors}, f, indent=2, default=str)

        browser.close()

if __name__ == "__main__":
    main()
